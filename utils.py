...
def load_data(dataset_source):
    ...
    #特征向量中性度
    # centrality = nx.eigenvector_centrality(dataset_source)
    # centrality = torch.FloatTensor(centrality)
...
#     #preds = output.max(1)[1].type_as(labels)
#     f1 = f1_score(labels, output, average='weighted')
#     return f1

def accuracy(output, labels):
    # print(output)
    preds = output.max(1)[1].type_as(labels)
    # print(labels) #5个类别分别20个
    # print(preds)
