...
# relation_network=RelationNetwork(nfeat=features.shape[1],
#             nhid=args.hidden,
#             dropout=args.dropout)
...
# optimizer_rnet = optim.Adam(scorer.parameters(),
#                        lr=args.lr, weight_decay=args.weight_decay)
...
def train(class_selected, id_support, id_query, n_way, k_shot):
    ...
    #关系网络,embedding向量是16维
    model = RelationWork(16, 8, n_way)
    #关系网络优化
    optimer = optim.Adam(model.parameters(), lr=0.01,weight_decay=5e-4)#优化器
    criterion = torch.nn.CrossEntropyLoss()  # 交叉熵
    # loss = nn.MSELoss(reduction="sum")
    time.time()
    optimer.zero_grad()
    out = model(prototype_embeddings, query_embeddings)
    output = torch.max(out, dim=1).indices
    labels_new = torch.Tensor([class_selected.index(i) for i in labels[id_query]])
...
def test(class_selected, id_support, id_query, n_way, k_shot):
    ...
    if args.cuda:
        labels_new = labels_new.cuda()
    loss_i = [0 if output[j] == labels_new[j] else 1 for j in range(100)]
    sum=0
    for i in range(0, len(loss_i)):
        sum += loss_i[i]
    # print(sum)

    # loss_test = loss_test.requires_grad_()
    # loss_test.backward()
    optimizer_encoder.step()
    optimizer_scorer.step()
    #relation module优化
